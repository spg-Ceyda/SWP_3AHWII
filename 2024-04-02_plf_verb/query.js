...
async function getUsersWhoHaveTrackInWatchlist(trackName) {
    const users = await prisma.benutzer.findMany({
        where: {
            watchlists: { some: { tracks: { some: { name: trackName } } } }
        },
        include: { watchlists: true }
    });
    return users;
...
async function main() {
    try {
        ...
        const watchlistsContainingTrack = await getWatchlistsContainingTrack(
            trackName
        );
        console.log(
            'Der Track ' + trackName + ' kommt auf folgenden Watchlists vor: ',
            watchlistsContainingTrack
        );

        const UsersWhoHaveTrackInWatchlist =
            await getUsersWhoHaveTrackInWatchlist(trackName);
        console.log(
            'Folgende Benutzer haben den Track ' +
                trackName +
                ' in einer ihrer Watchlists: ',
            UsersWhoHaveTrackInWatchlist
        );
    } catch (error) {
